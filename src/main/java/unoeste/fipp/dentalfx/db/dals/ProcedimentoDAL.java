...
public class ProcedimentoDAL implements IDAL<Procedimento> {
    ...
    public boolean gravar(Procedimento entidade) {
        ...
        sql=sql.replace("#2",String.format(Locale.US,"%.2f",entidade.getTempo()));
    ...
    public Procedimento get(int id) {
        ...
        try {
            if(resultSet.next()){
                procedimento=new Procedimento(resultSet.getInt("pro_id"),resultSet.getString("pro_desc"),
                        resultSet.getDouble("pro_tempo"), resultSet.getDouble("pro_valor"));
            }
    ...
    public List<Procedimento> get(String filtro) {
        ...
        try {
            while(resultSet.next()){
                Procedimento procedimento=new Procedimento(resultSet.getInt("pro_id"),resultSet.getString("pro_desc"),
                        resultSet.getDouble("pro_tempo"), resultSet.getDouble("pro_valor"));
